...
Description:
 - extract the  coordinate (key: $XYZ1,$XYZ2) and find charge and mutiplicity from the last paragragh (key: the 1st $STOP after last $XYZ1). 
...
LINK='%UseSSH\n%mem=100GB\n%nprocshared=28\n'
ROUTE='# b3lyp/6-31g(d,p) pop=npa EmpiricalDispersion=GD3BJ scf=(xqc,tight)'
#ROUTE='# td(NStates=50) um062x/6-31+g(d,p) pop=none scf=(xqc,tight)'
#ROUTE='# opt um062x/6-31+g(d,p) pop=npa scf=(xqc,tight)'
CHK='%chk='
    ...
    for t in tmpt:
        ...
        if len(ts)==2:
            try:
                ...
                continue
    print(':::>_<:::Couldn\'t find charge and multiplicity. Use 1 2 by default.You could change 50 in findchgmp function to a larger number to fix it.')
    return 1,2
