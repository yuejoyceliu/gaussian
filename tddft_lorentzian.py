...
#another way: count peaks in 700-210nm and S***2<2.6; then choose to keep or delete peaks out of range:
    '''
    ...
    outcome = chformat(allpeaks,realpeaks,spectra,calcst)
...
def chformat(allpeaks,realpeaks,result,calcst):
    title1 = ['Wavelength(nm)','Spectrum','T=Spec/maxSpec','Absorbance=-1og10(1-Spec)']
    title2 = ['MaxPeak(nm)','intensity','all S**2']
    title3 = ['MaxPeak(nm)','intensity','S**2<'+str(MAXSS2) +'&cutoff'+str(calcst)+'nm']
...
def addstates(fl):
    ...
    lk = []
    for l in lines:
        if l.startswith('%') and not l.lower().startswith('%lindaworker') and not l.lower().startswith('%oldchk'):
            lk.append(l)
        if l.startswith('#'):
            break
    rt = [l for l in lines if l.startswith('#')]
    if not bool(lk) or not bool(rt) or 'td' not in rt[-1].lower():
        print(':::>_<:::%s format not correct! Couldn\'t write restarted input file!' % gjf)
        return
    link = writelink(lk)
    route = writeroute(rt[-1])
